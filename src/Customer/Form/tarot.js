...
import { FORM_TAROT, FORM_TAROT_REMOVE } from '../../redux/actions/types'
...
export class Tarot extends Component {
    ...
    async componentDidMount() {
        const data = await getCards('tarot')
        let copy = [...data.cards]
        ...
        return (
            <View style={{ marginVertical: 20, paddingHorizontal: 10 }}>
                <View style={{ flexDirection: 'row', marginBottom: 20 }}>
                    ...
                    <Text style={{ flex: 1, fontSize: 15, fontWeight: '500', color: '#c1c1c1', textAlign: 'right' }}>{item.tarot.length + ' / 7'}</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
                    {
                        cards.map((x, i) => {
                            const selected = item.tarot.find(card => card === x.id)
                            console.log('selected', selected)
                            return (
                                <TouchableOpacity style={selected ? styles.tarotCardSelected : styles.tarotCard} onPress={() => {
                                    if (!selected && item.tarot.length === 7)
                                        return
                                    falAction(selected ? FORM_TAROT_REMOVE : FORM_TAROT, x.id)
                                }} key={i}>
                                    <Image source={icons.CartBack} style={{ width: '100%', height: '100%', borderRadius: 15 }} />
                                </TouchableOpacity>
                            )
                        })
                    }
